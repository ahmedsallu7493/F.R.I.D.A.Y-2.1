...
class FingerMouseController:
    def __init__(self,
        ...
        # MediaPipe setup (delayed import)
    ...
    def _process_frame(self):
        ...
        if results.multi_hand_landmarks:
            self._handle_gestures(img, results.multi_hand_landmarks[0])

        cv2.imshow("Finger Mouse", img)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            self.running = False

    def _handle_gestures(self, img, hand_landmarks):
        h, w, _ = img.shape
        ...
        coords = lambda lm: (int(lm.x * w), int(lm.y * h))
        x_index, y_index = coords(index)
        x_middle, y_middle = coords(middle)
        x_thumb, y_thumb = coords(thumb)
        x_pinky, y_pinky = coords(pinky)

        # Move cursor
        self._move_cursor(img, x_index, y_index, w, h)

        # Handle clicks
        if self._check_distance(x_middle, y_middle, x_thumb, y_thumb, self.click_threshold):
            pyautogui.click()
            print("🖱️ Click")

        # Exit condition
        if self._check_distance(x_pinky, y_pinky, x_thumb, y_thumb, self.click_threshold):
            cv2.putText(img, "EXIT", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            self.running = False

    def _move_cursor(self, img, x, y, w, h):
        """Smooth cursor movement with boundary checking"""
        cx, cy = w // 2, h // 2
        x1, y1 = cx - self.frame_reduction, cy - self.frame_reduction
        x2, y2 = cx + self.frame_reduction, cy + self.frame_reduction
        
        mapped_x = np.interp(x, (x1, x2), (0, self.screen_w))
        mapped_y = np.interp(y, (y1, y2), (0, self.screen_h))
        
        pyautogui.moveTo(mapped_x, mapped_y)
        cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)

    def _check_distance(self, x1, y1, x2, y2, threshold):
        return np.hypot(x1 - x2, y1 - y2) < threshold
...
class FridayUI(QMainWindow):
    ...
    def show_loading_screen(self):
        ...
        if not hasattr(self, 'loading_screen') or not self.loading_screen:
            ...
            # Load and setup GIF
            self.loading_movie = QMovie(os.path.join(self.graphics_path, "load.gif"))
