...
from Backend.whatappmsg import msg_from_query
...
class FridayCore:
    ...
    def handle_query(self, query):
        try:
            ...
            # Convert decision to lowercase command list
            decision_lower = [d.lower() for d in decision]

            # Filter only automation-related commands
            automation_tasks = [
                cmd for cmd in decision_lower
                if any(cmd.startswith(key) for key in ["open", "close", "play", "content", "google", "system"])
            ]
